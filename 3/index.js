...
const wireData = data.split('\n');
const wire1Data = wireData[0].split(',');
const wire2Data = wireData[1].split(',');

let currentCord = { x: 0, y: 0 };

function getWireCordinates(wireData) {
  const cords = {};
  let index = 0;
  wireData.forEach((data) => {
    const direction = data[0];
    const length = parseInt(data.substring(1));

    for (let i = 0; i < length; i++) {
      switch(direction) {
        case 'U': {
          currentCord = { x: currentCord.x,  y: currentCord.y + 1 };
          break;
        }
        case 'R': {
          currentCord = { x: currentCord.x + 1,  y: currentCord.y };
          break;
        }
        case 'D': {
          currentCord = { x: currentCord.x,  y: currentCord.y - 1 };
        ...
        default: {
          console.warn('whhhoops not suppose to be here');

        }
      }
      index += 1;
      cords[`[${currentCord.x}][${currentCord.y}]`] =  {
        x: currentCord.x,
        y: currentCord.y,
        index,
      }
...
const wire1Cords = getWireCordinates(wire1Data);
currentCord = { x: 0, y: 0 };
const wire2Cords = getWireCordinates(wire2Data);

let collidingWireCords = [];
Object.keys(wire1Cords).forEach((key) => {
  const cords = key.match(/(?<=\[).+?(?=\])/g);

  if (wire2Cords[`[${cords[0]}][${cords[1]}]`]) {
    collidingWireCords.push({ x: cords[0], y: cords[1] })
  }
});


let smallestDistance = 100000000;
collidingWireCords = collidingWireCords.map((cords) => {
  const distance = Math.abs(cords.x) + Math.abs(cords.y);
  if (distance < smallestDistance) { smallestDistance = distance; }
  return {
    x: cords.x,
    y: cords.y,
    distance,
    totalWireDistance: wire1Cords[`[${cords.x}][${cords.y}]`].index + wire2Cords[`[${cords.x}][${cords.y}]`].index,
  };
});
...
// Added index in getWireCordinates and just sort
collidingWireCords.sort((a,b) => a.totalWireDistance - b.totalWireDistance);

console.warn('Smallest total wire distance :', collidingWireCords[0].totalWireDistance);
// Answer: 107754